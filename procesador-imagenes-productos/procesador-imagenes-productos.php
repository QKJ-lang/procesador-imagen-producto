<?php
/**
 * Plugin Name: Procesador Imágenes de Productos
 * Description: Redimensiona imágenes a 1200x700 y las convierte a WebP con máxima calidad. Compatibilidad con WooCommerce
* Version: 2.0
...
// Función para redimensionar y convertir a WebP con máxima calidad
function redimensionar_y_convertir_a_webp($file_path, $target_width = 1200, $target_height = 700)
{
    // Obtener información de la imagen original
    $image_info = getimagesize($file_path);
    $mime_type = $image_info['mime'];

    // Configurar memoria para imágenes grandes
    ini_set('memory_limit', '256M');

    // Crear imagen según el tipo original con máxima calidad
    switch ($mime_type) {
        case 'image/jpeg':
        ...
        case 'image/png':
            ...
            // Preservar transparencia y calidad
    ...
    // Si la imagen ya tiene el tamaño exacto y es WebP, solo convertir si es necesario
    if ($original_width == $target_width && $original_height == $target_height && $mime_type == 'image/webp') {
        // Ya está en el formato correcto
        return $file_path;
    }

    // Calcular escala para mantener toda la imagen sin recortar
    $width_ratio = $target_width / $original_width;
    $height_ratio = $target_height / $original_height;

    // Usar la relación más pequeña para ajustar la imagen completa dentro del marco
    ...
    // Crear imagen en blanco del tamaño objetivo
    ...
    // Redimensionar con muestreo de alta calidad (sin recortar)
    imagecopyresampled(
        $target_image,
        $source,
        $x_offset,
    ...
    // Aplicar filtro de nitidez para mejorar calidad
    imagefilter($target_image, IMG_FILTER_CONTRAST, -5);

    // Generar ruta del archivo WebP
    $path_info = pathinfo($file_path);
    $webp_path = $path_info['dirname'] . '/' . $path_info['filename'] . '.webp';

    // Guardar como WebP con calidad 100 (máxima) y método de compresión lento pero de alta calidad
    imagewebp($target_image, $webp_path, 100);

    // Liberar memoria
    imagedestroy($source);
    imagedestroy($target_image);

    // Eliminar archivo original si es diferente al nuevo
...
// Procesar la imagen después de subirla
add_filter('wp_handle_upload', function ($upload) {
    ...
    $webp_path = redimensionar_y_convertir_a_webp($upload['file']);

    if ($webp_path && file_exists($webp_path)) {
        // Actualizar la ruta del archivo
        $upload['file'] = $webp_path;
        $upload['url'] = str_replace(
            pathinfo($upload['url'], PATHINFO_BASENAME),
            basename($webp_path),
            $upload['url']
        );
...
// Actualizar los metadatos con las dimensiones correctas
add_filter('wp_generate_attachment_metadata', function ($metadata, $attachment_id) {
    $metadata['width'] = 1200;
    $metadata['height'] = 700;
    $metadata['mime_type'] = 'image/webp';

    // Cambiar la extensión en el archivo
...
function render_productos_page()
{
    ...
    <div class="wrap">
        ...
        <p style="color:var(--text-muted)">Conversión automática a WebP + redimensionado 1200x700 (Máxima Calidad - Sin
            recortes)</p>
        ...
        <div class="glass">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="upload-box">
                    <input type="file" name="image" onchange="previewImage(event)" accept="image/*" required>
        ...
        '<?php echo isset($metadata['width']) ? $metadata['width'] : '1200'; ?>',
        '<?php echo isset($metadata['height']) ? $metadata['height'] : '700'; ?>'
                    ...
                    <div class="info">
                        ...
                        <span class="badge">WebP 1200x700</span>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
